...
public class DataProviders {
	...
	@DataProvider(name = "Computerdatabase")
	public Object[][] getComputerdatabase() {
		// Totals rows count
		int rows = obj.getRowCount("Computerdatabase");
		// Total Columns
		int column = obj.getColumnCount("Computerdatabase");
		int actRows = rows - 1;
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				data[i][j] = obj.getCellData("Computerdatabase", j, i + 2);
			}
		}
		return data;
	}
